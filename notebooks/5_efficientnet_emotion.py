...
def load_and_preprocess_image(image_path, target_size=(IMG_SIZE, IMG_SIZE)):
    ...
    try:
        ...
        # Converte para 3 canais (RGB) para EfficientNet
        img = cv2.cvtColor(img, cv2.COLOR_GRAY2RGB)
        
        # Normalização específica para EfficientNet (0-255 -> -1 a 1)
...
def create_efficientnet_model():
    """Cria modelo EfficientNet para classificação de emoções"""
    ...
    # Congela as camadas base inicialmente
    ...
    # Retorna tanto o modelo completo quanto o base_model para controle posterior
...
def fine_tune_model(model, base_model, X_train, y_train, X_val, y_val, monitor):
    """Fine-tuning do modelo EfficientNet"""
    # Primeira fase: treina apenas as camadas adicionadas
    ...
    history_1 = model.fit(
        ...
        callbacks=[early_stopping_1, reduce_lr_1, memory_callback],
        verbose=1
    )
    phase1_time = time.time() - phase1_start
    print(f"⏱️ Fase 1 concluída em: {timedelta(seconds=int(phase1_time))}")
    
    # Segunda fase: fine-tuning das últimas camadas
    print("\nFase 2: Fine-tuning das últimas camadas...")
    
    # Descongela as últimas camadas do EfficientNet
    base_model.trainable = True
    
    # Congela as primeiras camadas (deixa apenas as últimas 20 treináveis)
    ...
    history_2 = model.fit(
        ...
        epochs=EPOCHS,
        validation_data=(X_val, y_val),
        callbacks=[early_stopping_2, reduce_lr_2, memory_callback],
        verbose=1
    )
    phase2_time = time.time() - phase2_start
    print(f"⏱️ Fase 2 concluída em: {timedelta(seconds=int(phase2_time))}")
...
def plot_training_history(history):
    ...
    ax1.set_title('Model Accuracy - EfficientNet')
    ...
    ax2.set_title('Model Loss - EfficientNet')
...
def calculate_cross_dataset_drift(fer_features, raf_features):
    ...
    print(f"\nCross-Dataset Drift Analysis (EfficientNet):")
...
def data_augmentation(X_train, y_train):
    """Aplica data augmentation específico para EfficientNet"""
...
def save_training_report(monitor_stats, history, model_accuracy):
    ...
    report = f"""
# RELATÓRIO DE TREINAMENTO - EFFICIENTNET EMOTION CLASSIFIER
================================================================================
...
- Tempo Fase 1: {timedelta(seconds=int(history.get('phase1_time', 0)))}
- Tempo Fase 2: {timedelta(seconds=int(history.get('phase2_time', 0)))}
...
================================================================================
    ...
    print("📝 Relatório salvo em: training_report.txt")

def main():
    """Função principal"""
    print("Carregando EfficientNet para classificação de emoções...")
    ...
    try:
        ...
        print("Criando modelo EfficientNet...")
        model, base_model = create_efficientnet_model()
        model.summary()
        monitor.update_peak_memory()
        
        print("Treinando modelo com fine-tuning...")
        history = fine_tune_model(model, base_model, X_train, y_train, X_val, y_val, monitor)
        
        # Salva modelo
        model.save('efficientnet_emotion_model.h5')
        monitor.update_peak_memory()
        ...
        # Análise de drift (usando features intermediárias)
        ...
        print("Treinamento EfficientNet concluído!")
